...
| **Knockout Scoring** | Per-round format (Best of 3 / Straight set) and points target; locked once the knockout is generated |
...
4. **🔒 Knockout Rules Locked** — generating the knockout locks the scoring rules. From here the
   Settings controls are disabled and any attempt to change a rule is refused with a clear
   message. To change them again, clear the bracket (**Clear results only** or **Regenerate
   fixtures**), which unlocks the rules so you can reconfigure before starting again. Resetting a
   single knockout match keeps the bracket, so the rules stay locked.
